    ...
    def __call__(self, x, timesteps, context=None):
        ...
        with torch.no_grad():
            ...
            if self.asym:
                ...
                self.model.set_quant_state(weight_quant= (not self.rev) or (not self.act_quant), 
                                            act_quant= self.act_quant or self.rev) # 
                try:
                    _ = self.model(x, timesteps, context)
                except StopForwardException:
                    pass
        ...
        self.layer.set_quant_state((not self.rev) or (not self.act_quant) ,
                                    self.act_quant or self.rev)
        self.model.train()
